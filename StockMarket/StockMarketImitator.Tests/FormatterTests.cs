...
public class FormatterTests
{
    ...
    public void Alpha_HasNamedFields_NumberPrice_IsoTime()
    {
        ...
        Assert.Equal(187.30m, root.GetProperty("price").GetDecimal()); // a number, not a string
    ...
    public void Beta_PriceIsStringWithDot_TimeIsUnixMs()
    {
        ...
        Assert.Equal(JsonValueKind.String, price.ValueKind); // price is a string
        Assert.Equal("187.30", price.GetString());           // with a dot, not a comma
    ...
    public void Gamma_PositionalArray_UnixSeconds()
    {
        ...
        Assert.Equal(Tick.TimestampUtc.ToUnixTimeSeconds(), arr[3].GetInt64()); // seconds, not ms
